...
//            $this->error('验证码错误，请重试');
		...
		if(empty($_POST['password']) || empty($_POST['username'])){
			$this->ajaxReturn(array('status'=>0,'msg'=>'账号或者密码不能为空'));
		}
		if(!$admin){
			$this->ajaxReturn(array('status'=>0,'msg'=>'账号错误，请重试'));
		}
		if($admin['password']!=$pwd){
			$this->ajaxReturn(array('status'=>0,'msg'=>'密码错误，请重试'));
		}
		if($admin['status']==1){
			$this->ajaxReturn(array('status'=>0,'msg'=>'账号已锁定，请联系管理员'));
		}
        //记住账号
        ...
        Session('login',$admin);
        //跳转目标页
		$this->ajaxReturn(array('status'=>1,'msg'=>'账号已锁定，请联系管理员','redirect'=> U('Index/index')));
	}
    ...
    public function logout(){
        ...
        Session('login',null);
    	session('[destroy]');
    	$this->success('退出成功！', U('Login/index'),5);
    }
